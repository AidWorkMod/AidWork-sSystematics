...
public class GameControllerScript : MonoBehaviour
{
	...
	private void Start()
	{
        ...
        pickupParent = GameObject.Find("Items").transform;
        pickup = GameObject.Find("Items/Pickup_EnergyFlavoredZestyBar");
	...
	private void UseItem()
	{
		if (this.item[this.itemSelected] != 0)
		{
            ...
            else if (this.item[this.itemSelected] == 11) //Code for using the portal poster item
            {
                Ray ray7 = Camera.main.ScreenPointToRay(new Vector3((float)(Screen.width / 2), (float)(Screen.height / 2), 0f));
                RaycastHit raycastHit7;
                if (Physics.Raycast(ray7, out raycastHit7) && raycastHit7.collider.name.Contains("Window") && Vector3.Distance(playerTransform.position, raycastHit7.transform.position) <= 10f)
                {
    ...
    public void DropItem()
    {
        if (this.item[this.itemSelected] != 0)
        {
            GameObject droppedItem = Instantiate(pickup, playerTransform.position + playerTransform.forward * 3f, playerTransform.rotation, pickupParent);
            droppedItem.SetActive(true);
            PickupScript pickupScript = droppedItem.GetComponent<PickupScript>();
            pickupScript.gc = this;
            pickupScript.player = this.playerTransform;
            droppedItem.transform.GetChild(0).gameObject.GetComponent<SpriteRenderer>().sprite = Sprite.Create((Texture2D)this.itemTextures[this.item[this.itemSelected]], new Rect(0, 0, this.itemTextures[this.item[this.itemSelected]].width, this.itemTextures[this.item[this.itemSelected]].height), new Vector2(0.5f, 0.5f), this.itemTextures[this.item[this.itemSelected]].width); // Minor credits to PyroProGaming https://gamebanana.com/posts/10743300
            switch (this.item[this.itemSelected])
            {
                default:
                    break;
                case 1:
                    droppedItem.transform.name = "Pickup_EnergyFlavoredZestyBar";
                    break;
                case 2:
                    droppedItem.transform.name = "Pickup_YellowDoorLock";
                    break;
                case 3:
                    droppedItem.transform.name = "Pickup_Key";
                    break;
                case 4:
                    droppedItem.transform.name = "Pickup_BSODA";
                    break;
                case 5:
                    droppedItem.transform.name = "Pickup_Quarter";
                    break;
                case 6:
                    droppedItem.transform.name = "Pickup_Tape";
                    break;
                case 7:
                    droppedItem.transform.name = "Pickup_AlarmClock";
                    break;
                case 8:
                    droppedItem.transform.name = "Pickup_WD-3D";
                    break;
                case 9:
                    droppedItem.transform.name = "Pickup_SafetyScissors";
                    break;
                case 10:
                    droppedItem.transform.name = "Pickup_BigBoots";
                    break;
                case 11:
                    droppedItem.transform.name = "Pickup_Hammer";
                    break;
            }
            this.ResetItem();
        }
    ...
    public void Surprise()
    {
        ...
        this.badEndingMusic.Play(); //Stop the load noise
        this.baldi.SetActive(false); //Turns off Baldi
        this.principal.SetActive(false); //Turns off Principal
        this.crafters.SetActive(false); //Turns off Crafters
        ...
        this.firstPrize.SetActive(false); //Turns off Frist-Prize
        this.hideWall.SetActive(false);
